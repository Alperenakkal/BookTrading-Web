...
const Anasayfa = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-blue-100 flex flex-col items-center justify-start pt-16 pb-8 mt-28 font-overpass">
      <div className="text-center mb-12 animate-slide-down">
        <img
          alt="Your Company"
          src={logo}
          className="mx-auto h-28 w-auto rounded-lg mb-12 "
        />
        ...
        <div className="flex justify-center space-x-4">
          ...
          <button className="bg-white text-indigo-600 border border-indigo-600 rounded-full px-6 py-3 hover:bg-indigo-100 transition">
            <a href="/girişyap">Giriş Yap</a>
          </button>
        </div>
      </div>
      <div className="w-full max-w-4xl px-4 mb-12 flex-col items-center transform transition-transform duration-300 hover:scale-105 hover:shadow-l mt-28">
        <Carousel
          showArrows={true}
          autoPlay={true}
          infiniteLoop={true}
      ...
      <h2 className="text-2xl font-bold text-indigo-600 mb-6 animate-slide-down mt-28">
      ...
      <div className="w-full max-w-6xl px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {exampleBooks.map((book) => (
          <div
            key={book.id}
            className="bg-white p-4 rounded-3xl shadow-lg flex flex-col items-center transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl"
          >
            <img
              src={book.image}
              alt={book.title}
      ...
      <div className="w-full max-w-4xl px-4 mb-16 space-y-10 mt-28">
        {scrollRevealSections.map((section, index) => (
          <div
            key={index}
            className="bg-white p-6 rounded-3xl shadow-lg flex items-center gap-4 transform transition-all duration-500 ease-in-out opacity-0 translate-y-10 scroll-reveal"
          >
            <img
              src={section.image}
              alt={section.title}
      ...
      <div className="w-full max-w-4xl px-6 py-10 bg-white rounded-lg shadow-lg text-center mb-8 transition transform hover:scale-105 duration-500 ease-in-out mt-28">
        <h3 className="text-3xl font-extrabold text-purple-900 mb-6">
          Kitapağı ile Neler Yapabilirsiniz?
        </h3>
